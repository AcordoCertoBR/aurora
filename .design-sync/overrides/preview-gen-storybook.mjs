...
// AURORA FORK — see .design-sync/NOTES.md. The only change vs the bundled
// module is that COMPOSE resolves `parameters.backgrounds` and paints it
// behind the story, the way storybook's backgrounds addon does. Aurora's
// stories lean on that addon for contrast (Button.Negative*, Spinner.Negative,
// ChipBanner dark render WHITE content), so without it those previews are
// white-on-white while the storybook reference looks correct.
